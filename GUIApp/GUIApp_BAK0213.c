...
typedef struct {
  unsigned int	model;
  signed int		str_fine_tune;
	signed int		thrt_fine_tune;
	unsigned int	ch1_limit;
	unsigned int	ch2_limit;
	unsigned int	ch3_limit;
	unsigned int	ch4_limit;
}Typdef_ModelSave;

Typdef_ModelSave _ModelSave[4]=
{
...
static const GUI_WIDGET_CREATE_INFO _aDialogCreateWindow3[] = {
	{ WINDOW_CreateIndirect, 		"Window", 		ID_WINDOW_3, 							0, 		0, 		300, 	280, 		WM_CF_SHOW, 0,	0 },
  { DROPDOWN_CreateIndirect, 	"Dropdown", 	ID_DROPDOWN_MODELSEL, 		50, 	40, 	200, 	20, 		0, 0,	0 },
  ...
  { BUTTON_CreateIndirect, 		"CH3", 				ID_BUTTON_CH3, 						50, 	170, 	80, 	30, 		0, 0,	0 },
  { BUTTON_CreateIndirect, 		"CH4", 				ID_BUTTON_CH4, 						170, 	170, 	80, 	30, 		0, 0,	0 },
	{ TEXT_CreateIndirect, 			"MODEL SEL",	ID_TEXT_WIN3_1 , 					100, 	70, 	100, 	15, 		TEXT_CF_HCENTER|TEXT_CF_VCENTER ,0,0},
...
static const GUI_WIDGET_CREATE_INFO _aDialogCreateWinConfig[] = {
  { FRAMEWIN_CreateIndirect, "Config", ID_WINDOW_CONFIG, 0, 0, 300, 280, 0, 0x0, 0 },
	{ SPINBOX_CreateIndirect, "Spinbox", ID_SPINBOX_STR_LIMIT, 100, 50, 100, 30, 0, 0x0, 0 },
  { SPINBOX_CreateIndirect, "Spinbox", ID_SPINBOX_THRT_LIMIT, 100, 100, 100, 30, 0, 0x0, 0 },
	{ SPINBOX_CreateIndirect, "Spinbox", ID_SPINBOX_CHANNEL, 100, 150, 100, 30, 0, 0x0, 0 },
  { BUTTON_CreateIndirect, "OK", ID_BUTTON_CONFIG_OK, 75, 200, 50, 20 },
	{ BUTTON_CreateIndirect, "Cancel", ID_BUTTON_CONFIG_CANCLE, 300-75-50, 200, 50, 20 },
	{ TEXT_CreateIndirect, "STR LIMIT",0 , 20, 50, 80, 30, TEXT_CF_LEFT|TEXT_CF_VCENTER },
	{ TEXT_CreateIndirect, "THRT LIMIT",0 , 20, 100, 80, 30, TEXT_CF_LEFT|TEXT_CF_VCENTER },
	{ TEXT_CreateIndirect, "TX CHANNEL",0 , 20, 150, 80, 30, TEXT_CF_LEFT|TEXT_CF_VCENTER },
...
static const GUI_WIDGET_CREATE_INFO _aDialogCreateWinSave[] = {
  ...
  { BUTTON_CreateIndirect, "OK", ID_BUTTON_SAVE_OK, 75, 150, 50, 20 },
	{ BUTTON_CreateIndirect, "Cancel", ID_BUTTON_SAVE_CANCLE, 300-75-50, 150, 50, 20 },
};
...
static const GUI_WIDGET_CREATE_INFO _aDialogCreateWinSystem[] = {
  ...
  { BUTTON_CreateIndirect, "OK", ID_BUTTON_SYSTEM_OK, 75, 150, 50, 20 },
	{ BUTTON_CreateIndirect, "Cancel", ID_BUTTON_SYSTEM_CANCLE, 300-75-50, 150, 50, 20 },
};
      ...
      case WM_NOTIFICATION_SEL_CHANGED:
        ...
        if(Sel==0)				//Home
				...
				else if(Sel==1)		//Config
				{
					...
					_RC_hWindowConfig=GUI_CreateDialogBox(_aDialogCreateWinConfig, GUI_COUNTOF(_aDialogCreateWinConfig), _RC_cbFrameWinConfig, WM_HBKWIN, 300, 100);
				}
				else if(Sel==2)		//Save
				{
...
static void _RC_cbWindow2(WM_MESSAGE * pMsg) {
  ...
  case WM_PAINT:
		...
		GUI_Clear();
		GUI_SetColor(GUI_RED);
    GUI_DrawRoundedFrame(5,5,295,270,5,2);
...
static void _RC_cbWindow3(WM_MESSAGE * pMsg) {
	int NCode,i;
  int Id;
	WM_HWIN hDropdown, hSpinboxSTR, hSpinboxTHRT, hButtonCh3, hButtonCh4,hTEXT1,hTEXT2,hTEXT3;
  ...
  hTEXT1				= WM_GetDialogItem(hWin, ID_TEXT_WIN3_1);
	...
	case WM_INIT_DIALOG:
		//Dropdown 
		DROPDOWN_SetFont(hDropdown,GUI_FONT_20_ASCII);
		for(i=0;i<4;i++)
		{
			DROPDOWN_AddString(hDropdown, _RcDropdownString[i]);
		}
		//Spinbox
		SPINBOX_SetFont(hSpinboxSTR, GUI_FONT_20_ASCII);
		SPINBOX_SetFont(hSpinboxTHRT, GUI_FONT_20_ASCII);
		SPINBOX_SetRange(hSpinboxSTR,-100,100);
		SPINBOX_SetRange(hSpinboxTHRT,-100,100);
		SPINBOX_SetValue(hSpinboxSTR,0);
		SPINBOX_SetValue(hSpinboxTHRT,0);
		//Button
  ...
  case WM_NOTIFY_PARENT:
    ...
    case ID_DROPDOWN_MODELSEL: // Notifications sent by 'Dropdown'
      ...
      break;
    case ID_SPINBOX_STR: // Notifications sent by 'Spinbox'
      switch(NCode) {
      case WM_NOTIFICATION_CLICKED:
        // USER START (Optionally insert code for reacting on notification message)
        // USER END
      ...
      case WM_NOTIFICATION_VALUE_CHANGED:
        // USER START (Optionally insert code for reacting on notification message)
        //_ModelSave[0].str_fine_tune=SPINBOX_GetValue(hSpinboxSTR);
        break;
      // USER START (Optionally insert additional code for further notification handling)
      // USER END
      }
      break;
    case ID_SPINBOX_THRT: // Notifications sent by 'Spinbox'
      switch(NCode) {
      case WM_NOTIFICATION_CLICKED:
        // USER START (Optionally insert code for reacting on notification message)
        // USER END
      ...
      case WM_NOTIFICATION_MOVED_OUT:
        ...
        break;
      case WM_NOTIFICATION_VALUE_CHANGED:
        //_ModelSave[0].thrt_fine_tune=SPINBOX_GetValue(hSpinboxTHRT);
        break;
      // USER START (Optionally insert additional code for further notification handling)
      // USER END
      }
...
static void _RC_cbFrameWinConfig(WM_MESSAGE* pMsg) {
  int NCode,Id;
	WM_HWIN hSpinboxSTR, hSpinboxTHRT, hSpinboxCHANNEL;
	WM_HWIN hWin=pMsg->hWin;
  /* Get window handles for all widgets */

	hSpinboxSTR		= WM_GetDialogItem(hWin, ID_SPINBOX_STR_LIMIT);
	hSpinboxTHRT	= WM_GetDialogItem(hWin, ID_SPINBOX_THRT_LIMIT);
	hSpinboxCHANNEL	= WM_GetDialogItem(hWin, ID_SPINBOX_CHANNEL);
    ...
    NCode = pMsg->Data.v;
    switch(Id) {
    case ID_BUTTON_CONFIG_OK: 
      switch(NCode) {
				case WM_NOTIFICATION_CLICKED:		//save config
				
				WM_DeleteWindow(_RC_hWindowConfig);
			break;
			}
		break;
		case ID_BUTTON_CONFIG_CANCLE: 
      switch(NCode) {
				case WM_NOTIFICATION_CLICKED:
				WM_DeleteWindow(_RC_hWindowConfig);
			break;
	...
	break;
	case WM_INIT_DIALOG:
		SPINBOX_SetValue(hSpinboxSTR,70);
		SPINBOX_SetValue(hSpinboxTHRT,70);
		SPINBOX_SetValue(hSpinboxCHANNEL,88);
	break;
  case WM_PAINT:
...
static void  _RC_CreateWindow(void)
{
  ...
  //
  _cbBkWindowOld = WM_SetCallback(WM_HBKWIN, _RC_cbBkWindow);
  //
  // Create windows
  //
  _RC_hWindow1 = WM_CreateWindow( 0,  0, 200, 480, WM_CF_SHOW | WM_CF_MEMDEV,	_RC_cbWindow1, 0);
  _RC_WM1_Iconview();
  _RC_hWindow2 = WM_CreateWindow(200, 0, 300, 280, WM_CF_SHOW | WM_CF_MEMDEV,	_RC_cbWindow2, 0);

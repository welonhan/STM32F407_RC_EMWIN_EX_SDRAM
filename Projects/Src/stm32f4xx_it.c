...
{
	if(__HAL_GPIO_EXTI_GET_IT(TOUCH_INT_PIN) != RESET)
  {
    __HAL_GPIO_EXTI_CLEAR_IT(TOUCH_INT_PIN);
    HAL_GPIO_EXTI_Callback(TOUCH_INT_PIN);
  }
	else if	(__HAL_GPIO_EXTI_GET_IT(RF24L01_IRQ_GPIO_PIN) != RESET)
  {
    __HAL_GPIO_EXTI_CLEAR_IT(RF24L01_IRQ_GPIO_PIN);
    HAL_GPIO_EXTI_Callback(RF24L01_IRQ_GPIO_PIN);
  }
}
